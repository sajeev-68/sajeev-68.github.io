...
Hello Everyone! I know its been a while(again) but I was kinda stuck and I couldn't find a way forward (I'll explain it better later in this post).
...
*I take no responsibility or accountability for infection of malicious software, programs, or files onto any computer or workstation. These projects are for educational purposes only. I do not condone the development, use, or spreading of programs to harm assets, networks, or individuals intentionally.*

*I do not own the malware/program nor am I claiming to be the program's author all rights and ownership go to the person/team who designed it.*
...
- The malware, using refelective code loading, was able to run a different process. I managed to dump the file and fix it to make it into a working executable.
...
- Before going into the default, first let me explain how this is structured
...
- Going into the function we can see a CreateProcessW, this function creates the same process except this time it calls it with the command line parameter.
...
- Anyways, we see calls to 2 fucntion, one function retrives the VoulmeInformation and gets the Windows Directory.

- The next is the more important function, previously we discussed how this program creates string using heap and snwprintf, here we can observe how this string is retrived.
...
- Here we can see that the string that is to be decrypted is stored in `ecx` 4 bytes at a time and then it is XOR'ed with `edx` which contains the key.

![XORdata1](/assets/img/blog_imgs/87.png)
![XORdata1](/assets/img/blog_imgs/87-1.png)

- As we can see the data that is stored is in little endian so if we convert it we can see that it says "Glob\al" 
...
- Here a mutex is created in order to protect the system from re-infection, if you have the same program running in the background this mutex will not be created and it can know that this specific computer has already been infected.
...
- Next Starts the service using StartServiceW.

![objs](/assets/img/blog_imgs/93.png)

- We can view the services set up to check if there is a services called tonerdurable using the services sysutil tool

- Lo and Behold we can see that there is a service set up called tonerdurable

![objs](/assets/img/blog_imgs/94.png)

- After starting the service we exit the program. This service starts the program again except it starts the program from the SYSWOW64 folder.
...
- However, since we are executing the program from our local copy, we need to make sure that lstrcmpiW returns 0 so that the program thinks that it is the copy from the SYSWOW64 directory.
...
- Next we move into a function which creates a hash object. probably to hash some data.

![objs](/assets/img/blog_imgs/95.png)

- Again, after we exit this function we go into the next stage where the program gets all the current running process using the CreateToolhelp32Sanpshot.
...
- all the servers that recive this data, the C2 servers, are all probably shutdown, so getting to the next stage is not possible. This is the major problem that I have been facing. Without these servers being, the next stage of the malware is not obtainable.

- Using FakeNet, we can see the data which is sent to this C2 server.

![objs](/assets/img/blog_imgs/100.png)

- The program, runs continously in the background, sending data to a bunch of C2 servers.

- After sending the data, I presume that the program sends over the data and recive a file back, if the program recives the correct file it proceeds to create a new process using the recived data. But I cannot confirm this, as there is not a way to recive the file for nex phase.

- With that being said, This concludes my analysis of the Emotet malware.


### Future works

- Anyways, I noticed a major flaw in getting malware samples from the internet, these samples are often well analyzed and documented. Because of this the C2 servers are shutdown(for good reasons) so geiing the next stage is next to impossible.

- To combat this I am collecting my own samples using a honeypot.

- I have set up this honeypot(Tpot) in google cloud. there are may guides online set up these honeypots.

![objs](/assets/img/blog_imgs/101.png)

- I have obtained several samples of malware both for linux and windows(btw this honeypot has been up for only about 2 weeks). Although most of them are wannacry/wannacrypt, which is surprising, its a pretty old malware. In the linux side of things I am woefully under-equipped to analyze but, it does seem interesting, most samples are multiverze or xorddos.

![objs](/assets/img/blog_imgs/102.png)

- I also have simple python script to get the details of the files that are captured in the honeypot, using virustotal([link](https://github.com/sajeev-68/vt-dionaea-scanner)), it only scans the dionea honeypot but i plan on adding other directories in the future.
...
](https://www.youtube.com/watch?v=V1k-6_LiSKw)
